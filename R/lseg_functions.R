...
generate_lseg_hset <- function(dfTMP, dfRAD){
  ...
  dailyMetrics <- sqldf("SELECT year, month, day, max(temp) max_temp, min(temp) min_temp
                       FROM dfTMP
                       GROUP BY year, month, day")
  # repeat daily maximum and minimum 24 times to match up with hourly values
  hourlyMetrics <- sqldf(
    "select a.year, a.month, a.day, a.hour, a.temp, b.min_temp, b.max_temp 
     from dfTMP as a left outer join dailyMetrics as b 
     on (a.year = b.year and a.month = b.month and a.day = b.day)
     order by a.year, a.month, a.day, a.hour
    "
  )
  # calculate PET (mm/day)
  # AND convert PET to inch/hour
  k <- 0.19 # empirical coefficient
  hourlyMetrics$PET <- (
    0.0135*(k) * (hourlyMetrics$temp+17.8)
    * ((hourlyMetrics$max_temp - hourlyMetrics$min_temp)^0.5)
    *0.408*(dfRAD$rad)
  )*0.0393701/24

  # create PET dataframe
  dfHSET <- sqldf("select year, month, day, hour, PET from hourlyMetrics")
  return(dfHSET)
}
