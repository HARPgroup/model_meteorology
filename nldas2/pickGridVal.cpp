...
void 
getExtent(GDALDataset * temp, double *ranges)
{
	//cout << "IN : getExtent\n";
...
double 
getRasterValue(GDALDataset * layer, int bandNumber, int x, int y)
{
	//cout << "IN : getRasterValue\n";
	//cout << "Xres= " << Xres << "\n";
	//cout << "Yres= " << Yres << "\n";
	//? ? cout << "X Size= " << layer->GetRasterXSize() << "\n";
	//? ? cout << "Y Size= " << layer->GetRasterYSize() << "\n";
	//? ? cout << "Count = " << layer->GetRasterCount() << "\n";
	//GDALDataset * layer;
	//GDALAllRegister();
	...
	int             col = y;
	//(int) floor((x - ranges[0]) / ranges[4]);
	int             row = x;
	//(int) floor((ranges[3] - y) / ranges[5]);
	//cout << "col = " << col << "\n";
	//cout << "row = " << row << "\n";
...
/*
 * main(){ GDALDataset* temp; GDALAllRegister(); temp =
 * (GDALDataset*)GDALOpen("NLDAS_FORA0125_H.A19790101.1300.002.grb",
 * GA_ReadOnly);
 * 
 ...
 * "<<temp->GetRasterCount()<<"\n"; //cout<<"Count =
 * "<<temp->GetRasterCount()<<"\n"; //GDALDataType data[3]; //
 * *GDALRasterBand *band = temp->GetRasterBand(1); cout<<"X     =
 * "<<band->GetXSize()<<"\n"; GDALDataType type = band->GetRasterDataType();
 * int size = GDALGetDataTypeSize(type) / 8; void *data = CPLMalloc(size);
 * //temp->RasterIO(GF_Read, 1, 1, 1, 1, data, 1, 1, type, 1, NULL, 0, 0, 0);
 * band->ReadBlock(0, 1, data); cout<<"data= "<<readValue ( data, type, 0 );
 * //for(int i=0; i<3; i++) //	cout<<"data= "<<data[i]<<"\n";
 * 
 * 
 * cout<<"\n*********************\n"; getExtent(temp);
